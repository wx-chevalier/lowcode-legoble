...
engine.addFact('account', (params: any, almanac: any) => {
  // get account list
  return almanac.factValue('accounts').then((accounts: any) => {
    // use "params" to filter down to the type specified, in this case the "customer" account
    let customerAccount = accounts.filter((account: any) => account.type === params.accountType);
    // return the customerAccount object, which "path" will use to pull the "balance" property
    return customerAccount[0];
  });
});

engine.addFact('product', (params: any, almanac: any) => {
  // get product list
  return almanac.factValue('products').then((products: any) => {
    // use "params" to filter down to the product specified, in this case the "giftCard" product
    let product = products.filter((product: any) => product.productId === params.productId);
...
let facts: any;
engine
  .on('success', (event: any, almanac: any) => {
    console.log(
      facts.userId +
        (' DID ' as any).green +
